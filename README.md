...
Co-authored by Steven C., age 12.
...
| `src/input.ts` | `createSteeringInput()` — normalized mouse/touch steering, composed with the optional tilt module. Touch temporarily overrides tilt while held. |
| `src/tilt.ts` | `TiltSteering` — experimental phone-tilt steering, fully self-contained for easy removal. Roll = gravity measured in a tangent frame anchored at the calibrated grip; pitch = gyro-referenced rotation about the screen x-axis via `RelativeOrientationSensor` (falls back to the gravity frame). Prefers `GravitySensor`, falls back to filtered `devicemotion`; calibration is stability-gated so the button tap doesn't corrupt the center. |
